// ============================================================
// FILE: js/managers/dailyrewards.js
// ============================================================
...
export default class DailyRewardManager {
    ...
    claimDailyReward() {
        ...
        this.streak = Sanitizer.clamp(this.streak, 0, 365);
        ...
        if (this.streak === 7) {
            ...
            reward = rewards[Math.min(idx, rewards.length - 1)];
        }

        this.lastClaimDate = today;
        this.claimedToday = true;

        if (reward.particles) this.resourceManager.addParticles(Sanitizer.sanitizeNumber(reward.particles, 0));
        if (reward.relics) this.resourceManager.addRelics(Sanitizer.sanitizeNumber(reward.relics, 0));
        if (reward.artifacts) this.resourceManager.addArtifacts(Sanitizer.sanitizeNumber(reward.artifacts, 0));
    ...
    fromJSON(data) {
        ...
        this.streak = Sanitizer.clamp(Sanitizer.sanitizeNumber(data.streak, 0), 0, 365);
