// ============================================================
// FILE: managers/skilltree.js – FIXED: Prestige-Reset für Punkte
// ============================================================
...
export default class SkillTreeManager {
  constructor(eventBus, hero) {
    ...
    // ---------- FIX: Prestige-Reset für Skills ----------
    this.eventBus.subscribe(EVENTS.HERO_PRESTIGE, this._onPrestige.bind(this));
  }

  _onPrestige() {
  // Skills bleiben erhalten, aber Prestige-Punkte werden im Hero zurückgesetzt
  // Hero.prestigePoints wird bei Prestige um 1 erhöht, das ist okay
  }
  ...
  unlockSkill(skillId) {
    ...
    for (const req of skill.req) {
      if (!this.hero.unlockedSkills.includes(req)) return { success: false, message: 'Voraussetzung nicht erfüllt.' };
    }

    this.hero.prestigePoints -= skill.cost;
