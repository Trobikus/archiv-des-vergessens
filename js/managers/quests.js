...
export default class QuestManager {
    constructor(eventBus, hero, resourceManager, clanManager) {
        ...
        this.eventBus.subscribe(EVENTS.QUEST_MANUAL_GATHER, () => {
            this._checkDailyReset();
            this.dailyQuests.gatherClicks = (this.dailyQuests.gatherClicks || 0) + 1;
            this.checkCurrentQuest();
        });
        this.eventBus.subscribe(EVENTS.EXPEDITION_COMPLETE, () => {
            this._checkDailyReset();
            this.dailyQuests.expeditions = (this.dailyQuests.expeditions || 0) + 1;
            this.checkCurrentQuest();
        });
        this.eventBus.subscribe(EVENTS.FORGE_CRAFTED, () => {
            this._checkDailyReset();
            this.dailyQuests.craftedItems = (this.dailyQuests.craftedItems || 0) + 1;
            this.checkCurrentQuest();
        });
    ...
    _grantReward(rewardObj) {
        if (rewardObj.particles) this.resourceManager.addParticles(Sanitizer.sanitizeNumber(rewardObj.particles, 0));
        if (rewardObj.relics) this.resourceManager.addRelics(Sanitizer.sanitizeNumber(rewardObj.relics, 0));
        if (rewardObj.artifacts) this.resourceManager.addArtifacts(Sanitizer.sanitizeNumber(rewardObj.artifacts, 0));
        if (rewardObj.memoryDust) this.resourceManager.addMemoryDust(Sanitizer.sanitizeNumber(rewardObj.memoryDust, 0));
    }
    ...
    claimReward() {
        if (this.isCurrentQuestComplete()) {
            ...
            this.questIndex = Sanitizer.clamp(this.questIndex + 1, 0, this.mainQuests.length);
            this.eventBus.publish(EVENTS.HERO_UPDATED);
            this.eventBus.publish(EVENTS.QUEST_UPDATED);
        }
    }
    ...
    getDailyQuests() {
        ...
        return this.dailyDefs.map(def => {
            const progress = Sanitizer.sanitizeNumber(this.dailyQuests[def.key] || 0, 0);
    ...
    fromJSON(data) {
        if (!data) return;
        this.questIndex = Sanitizer.clamp(Sanitizer.sanitizeNumber(data.questIndex, 0), 0, this.mainQuests.length);
        this.dailyQuests = {
            date: data.dailyQuests?.date || '',
            gatherClicks: Sanitizer.sanitizeNumber(data.dailyQuests?.gatherClicks, 0),
            expeditions: Sanitizer.sanitizeNumber(data.dailyQuests?.expeditions, 0),
            craftedItems: Sanitizer.sanitizeNumber(data.dailyQuests?.craftedItems, 0),
            claimed: Sanitizer.sanitizeArray(data.dailyQuests?.claimed, [])
        };
    }
}