// ============================================================
// FILE: js/ui/heroui.js – Helden-UI (AAA-RPG)
// ============================================================
...
export default class HeroUI extends BaseModalUI {
    constructor(context) {
        ...
        // Stat-Cache für differenzielles Rendering
        this._statsCache = {
            raw: null,
            html: '',
            statPoints: 0
    ...
    _renderStats(previewItem) {
        ...
        const currentHash = JSON.stringify({ attr: simAttr, combat: simCStats, points: this.hero.unspentStatPoints });
        if (this._statsCache.raw === currentHash && !previewItem) return;
        this._statsCache.raw = currentHash;
        ...
        if (this._statsCache.html !== html) {
            this._statsCache.html = html;
    ...
    _renderResourcesTab() {
        ...
        container.innerHTML = `
            <div class="glass-inner-panel mb-2">
                ...
                <div class="flex-between mb-1"><span class="text-muted">Mneme-Partikel:</span> <span class="text-gold text-bold">${formatNumber(res.particles)}</span></div>
                <div class="flex-between mb-1"><span class="text-muted">Mneme-Relikte:</span> <span class="text-gold text-bold">${formatNumber(res.relics)}</span></div>
                <div class="flex-between mb-1"><span class="text-muted">Mneme-Artefakte:</span> <span class="text-gold text-bold">${formatNumber(res.artifacts)}</span></div>
                <div class="flex-between"><span class="text-muted">Erinnerungsstaub:</span> <span class="text-dust text-bold">${formatNumber(res.memoryDust)}</span></div>
            </div>
            <div class="glass-inner-panel mb-2">
                <h3 class="options-header cinzel text-sm" style="margin-bottom: 0.8rem;">Heldentum & Prestige</h3>
                <div class="flex-between mb-1"><span class="text-muted">Prestige-Stufe:</span> <span class="text-gold text-bold">Stufe ${this.hero.prestigeLevel}</span></div>
                <div class="flex-between mb-1"><span class="text-muted">Prestige-Boni:</span> <span class="text-success text-sm" style="text-align: right;">+${this.hero.getPrestigeBonus('particleStart')} Startpartikel<br>+${this.hero.getPrestigeBonus('jobRate')}% Sammelrate<br>+${this.hero.getPrestigeBonus('relicChance')}% Reliktchance</span></div>
                <div class="flex-between mt-1 pt-1" style="border-top: 1px solid rgba(255,255,255,0.03);"><span class="text-muted">Exklusive Items:</span> <span class="text-gold text-bold text-sm" style="max-width: 60%; text-align: right;">${prestigeItems || 'Keine'}</span></div>
            </div>
            <div class="glass-inner-panel">
                <h3 class="options-header cinzel text-sm" style="margin-bottom: 0.8rem;">Statistiken & Ziele</h3>
                <div class="flex-between mb-2"><span class="text-muted">Besiegte Bosse:</span> <span class="text-highlight text-bold">${this.hero.defeatedBosses.length}</span></div>
                ${achievementsHtml ? `<div class="border-t-light pt-2 mt-2"><div class="text-gold cinzel text-sm mb-1" style="letter-spacing: 0.5px;">Erfolgsfortschritt</div>${achievementsHtml}</div>` : ''}
                ${dailyState ? `<div class="border-t-light pt-2 mt-2"><div class="text-gold cinzel text-sm mb-1" style="letter-spacing: 0.5px;">Tägliche Serie</div><div class="flex-between text-sm text-muted"><span>Streak-Zähler:</span> <span class="text-bold text-highlight">${dailyState.streak} Tage</span></div></div>` : ''}
            </div>
        `;
    }
    ...
    _renderEquipmentTab() {
        ...
        const rarityMap = { common: 'Gewöhnlich', uncommon: 'Ungewöhnlich', rare: 'Selten', epic: 'Episch', legendary: 'Legendär' };
        ...
        salvageBtn.addEventListener('click', () => {
            ...
            const itemsToSalvage = selectedRarity === 'all' ? items : items.filter(item => item.rarity === selectedRarity);
            ...
            if (salvaged > 0) {
                this.eventBus.publish(EVENTS.UI_ADD_LOG, { text: `${salvaged} ${rarityLabel} Item(s) verwertet. +${dustGained} Staub.`, type: 'event' });
                this.eventBus.publish(EVENTS.HERO_UPDATED);
