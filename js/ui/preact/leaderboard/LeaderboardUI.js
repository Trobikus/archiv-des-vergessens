...
export function LeaderboardUI({ stateManager, eventBus, services }) {
  ...
  const { isOpen, close } = useModal(eventBus, {
    openEvent: EVENTS.UI_OPEN_LEADERBOARD,
    onOpen: (data) => {
      const targetTab = data && data.tab ? data.tab : 'personal';
      setActiveTab(targetTab);
      if (targetTab === 'global') fetchGlobal();
    }
  });
  ...
  return html`
    <${ModalShell}
      isOpen=${isOpen}
      onClose=${close}
      title="🏛️ Das Große Archiv"
      titleClass="modal-title glow-text cinzel text-center mb-1"
      contentStyle="width: 600px; max-width: 95vw;"
    >
                ...
                ${globalEntries.map((entry, index) => {
                  ...
                  return html`
                    <div class="leaderboard-row glass-panel"
                      ...
                      <span class="text-gold text-bold" style="text-align: right; font-size: 0.95rem;">P${entry.prestige}</span>
                      <span class="text-muted text-sm" style="text-align: right; font-size: 0.8rem;">${entry.bosses} (${entry.level})</span>
                    </div>
                  `;
                })}
              </div>
