// ============================================================
// FILE: js/controllers/navigation.js – mit Button-Update-Fix
// ============================================================
    ...
    bindEvents() {
        ...
        safeAddEventListener(optHardReset, 'click', async () => {
            if (confirm('WARNUNG! Möchtest du deinen kompletten Spielstand UNWIDERRUFLICH löschen?')) {
                if (this.callbacks.onHardReset) {
                    await this.callbacks.onHardReset();
                }
            }
        });
    }
    ...
    async updateMenuButtons() {
        ...
        try {
            ...
            if (this.elements.btnContinue) {
                ...
                this.elements.btnContinue.style.display = hasSave ? 'inline-block' : 'none';
            }
            if (this.elements.btnNewGame) {
                this.elements.btnNewGame.style.display = hasSave ? 'none' : 'inline-block';
            }
            if (!hasSave && this.elements.btnContinue) {
                this.elements.btnContinue.disabled = true;
            }
        } catch (error) {
            console.error('[Navigation] updateMenuButtons fehlgeschlagen:', error);
            if (this.elements.btnNewGame) this.elements.btnNewGame.style.display = 'inline-block';
            if (this.elements.btnContinue) {
                this.elements.btnContinue.disabled = true;
