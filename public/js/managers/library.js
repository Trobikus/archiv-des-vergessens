...
export default class LibraryManager {
  ...
  getBonus(id) {
    const level = this.upgrades[id].level;
  ...
  toJSON() {
    const data = {};
    for(let key in this.upgrades) data[key] = this.upgrades[key].level;
    return data;
  }
  ...
  fromJSON(data) {
    ...
    for(let key in this.upgrades) {
        if(data[key] !== undefined) this.upgrades[key].level = data[key];
    }
  }
}