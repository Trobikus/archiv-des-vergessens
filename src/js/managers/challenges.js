...
export default class ChallengeManager {
  ...
  startChallenge(challengeId) {
    if (this.hero.bossProgress > 0) return { success: false, message: 'Anomalien können nur direkt nach einem Prestige (Boss 0) gestartet werden.' };
    if (this.activeChallenge) return { success: false, message: 'Es läuft bereits eine Anomalie.' };
    if (this.completedChallenges.includes(challengeId)) return { success: false, message: 'Diese Anomalie wurde bereits gemeistert.' };
